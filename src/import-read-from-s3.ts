...
function transform(
  ...
  return {
    ...
    genres: tmdbMovie.genre_ids
      ? tmdbMovie.genre_ids.map((genreId) => genresMap.get(genreId)).join(', ')
      : undefined,
...
export const handler = async (event: any): Promise<any> => {
  const genres: TmdbGenre[] = (await (
    await fetch('https://storage.yandexcloud.net/movies-app/genres.json')
  ).json()) as TmdbGenre[];
  const genresMap = new Map(genres.map((genre) => [genre.id, genre.name]));
  const tmdbMovies: TmdbMovie[] = (await (
    await fetch('https://storage.yandexcloud.net/movies-app/tmdb.json')
  ).json()) as TmdbMovie[];

  for (let i = 0; i < MOVIES_COUNT; i++) {
    const movie = transform(tmdbMovies[i], genresMap);
...
async function importImage(
  ...
  const uploadParams: PutObjectCommandInput = {
    ...
    Key: `${folder}/${file}`,
    Body: await (
      await fetch(`https://image.tmdb.org/t/p/${size}/${file}`)
    ).buffer(),
  };
