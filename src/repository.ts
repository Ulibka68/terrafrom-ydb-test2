...
export async function batchWriteMovies(
  movies: Movie[]
...
export async function getMovieById(
  id: number
): Promise<Movie | undefined | ApiError> {
  ...
  try {
    const result: GetItemCommandOutput = await callWithToken(() =>
      ddbClient.send(new GetItemCommand(params))
    );
...
export async function getMovies(limit?: number): Promise<Movie[] | ApiError> {
  ...
  try {
    const result: QueryCommandOutput = await callWithToken(() =>
      ddbClient.send(new QueryCommand(params))
    );
    return result.Items
      ? result.Items.map((value) => unmarshall(value) as Movie)
      : [];
  } catch (e) {
...
export async function putVote(vote: Vote): Promise<Vote | ApiError> {
  ...
  try {
    ...
    console.debug(
      `Added new vote with user_id=${vote.user_id}, movie_id=${vote.movie_id}, value=${vote.value}`
    );
    return vote;
  } catch (e) {
...
export async function getVote(
  userId: string,
  movieId: number
): Promise<Vote | undefined | ApiError> {
  const params: GetItemCommandInput = {
    TableName: VOTES_TABLE,
...
export async function getVotes(movieId: number): Promise<Vote[] | ApiError> {
  ...
  try {
    ...
    return result.Items
      ? result.Items.map((value) => unmarshall(value) as Vote)
      : [];
  } catch (e) {
...
export async function updateRating(
  ...
  try {
    await callWithToken(() => ddbClient.send(new UpdateItemCommand(params)));
    console.debug(
      `Updated rating for movie id = ${movieId}, new rating = ${rating}`
    );
    return rating;
  } catch (e) {
